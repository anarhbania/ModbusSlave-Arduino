...
ModbusSlave::ModbusSlave(HardwareSerial *_port, uint32_t _baud, uint8_t _slaveID, uint16_t _registersAddress, uint16_t *_registers, uint16_t _registersSize, uint16_t _timeout)
{
	port = _port;
	slaveID = _slaveID;
...
{	
	...
	else if(millis() - lastTimeout > 1000)
	{
